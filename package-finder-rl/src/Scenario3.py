...
def main():
    ...
    for epoch in tqdm(range(EPOCHS)):
        ...
        while True:
            ...
            # +1 reward if we reached terminal state correctly (r->g->b),
            # -1 reward if we reached terminal state incorrectly (e.g. g->)
            #  -0.001 reward for a step not into terminal state
            # -0.01 reward for a step into wall
            reward = (
                (isTerminal and newPackagesLeft == 0)
                - 1 * (isTerminal and newPackagesLeft > 0)
                - 0.001 * (not isTerminal)
                - 0.01 * (newPos == currentPos)
            )
            ...
            if not isTerminal:
                ...
                break
        # decay epsilon every few epochs

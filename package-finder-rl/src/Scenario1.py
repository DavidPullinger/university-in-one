...
ACTIONS = 4
X = Y = 13
q_table = np.zeros((X, Y, ACTIONS))


def calc_stochastic_q(action, currentPos):
    q_sum = 0
    unintended_actions = [0, 1, 2, 3]
    unintended_actions.remove(action)
    for unintended_act in unintended_actions:
        q_sum += (
            0.8 * q_table[currentPos][action]
            + 0.2 * q_table[currentPos][unintended_act]
        )
...
def main():
    ...
    for epoch in tqdm(range(EPOCHS)):
        ...
        # loop until wwe get to terminal state
        while True:
            ...
            if np.random.random() > EPSILON:
                ...
                if not stochastic:
                    action = np.argmax(q_table[currentPos])
                # if they are, we use the probability of each action to choose max
                else:
                    action = 0
                    for act in range(ACTIONS):
                        if calc_stochastic_q(act, currentPos) > calc_stochastic_q(
                            action, currentPos
                        ):
                            action = act
            else:
            ...
            reward = (
                100 * isTerminal - 1 * (not isTerminal) - 1 * (newPos == currentPos)
            )

            if not isTerminal:
                # max possible Q value in next step
                max_future_q = np.max(q_table[newPos])
                # current Q value
                current_q = q_table[currentPos + (action,)]
                # calculate new q
                new_q = (1 - LEARNING_RATE) * current_q + LEARNING_RATE * (
                    reward + DISCOUNT * max_future_q
                )
                # update Q table with new Q value
                q_table[currentPos + (action,)] = new_q
            # if we have reached terminal state
            else:
                q_table[currentPos + (action,)] = reward
                break
        # decay epsilon

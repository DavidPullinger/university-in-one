...
// NOTE: The WaveFront OBJ format spec, states that meshes are allowed to be
// defined by faces
//       consisting of 3 or more vertices. For the purposes of this loader (and
//       since this is the most common case) we only support triangle faces
//       (defined by exactly 3 vertices) and as a result, the loader will ignore
//       any data after the third vertex for each face
//
//       Similarly, the spec allows for vertex positions and texture coordinates
//       to both have a w-coordinate. The loader will ignore these and assumes
//       that all vertex specifications contain exactly 3 values, and that all
//       texture coordinate specifications contain exactly 2 values

// NOTE: There is currently no support for mtl material references or anything
// like that,
...
void GeometryData::loadFromOBJFile(string filename) {
  ...
  while (!inStream.eof()) {
    ...
    case NONE: {
      ...
      if (typeChar1 == '#') {
        ...
        cout << "OBJ parse error: Expected 'v', 'f' or '#' at the start of the "
                "line"
             << endl;
        ...
        if ((typeChar2 == ' ') || (typeChar2 == '\t')) {
          ...
          cout << "OBJ parse error: Free-form geometry is not supported, "
                  "ignoring"
               << endl;
          currentDataType = COMMENT;
        } else {
          cout << "Unsupported data entry v" << (char)typeChar2 << ", ignoring"
               << endl;
          currentDataType = COMMENT;
        }
      }
    ...
    case FACE: {
      // NOTE: Here is where we assume that exactly 3 vertices are used to
      // specify a face
      int vertIndex = 0;
      int texCoordIndex = 0;
      int normalIndex = 0;
      ...
      for (int index = 0; index < 3; index++) {
        ...
        if (postVertexCheckChar == '/') {
          ...
          if (postTexCoordCheckChar == '/') {
            ...
            inStream
                .unget(); // This is just to prevent us from consuming a newline
          }
        } else {
          inStream.unget();
        }
  ...
  // NOTE: Since our rendering pipeline supports only 1 set of indices for our
  // data, we need to
  //       do some post-processing here in order to lay out all the unique
  //       v/vt/vn triples
  // TODO: We're currently just assuming all the triples are distinct, but its
  // probably worth doing
  //       at least a little bit of checking on that front
  // TODO: We're deciding whether or not to add texture coords and normals on a
  // per-face basis,
  //       which doesn't really make sense because if there are any then there
  //       should be for all vertices, but this way that might not be the case
  for (int faceIndex = 0; faceIndex < tempGeom.faces.size(); faceIndex++) {
    ...
    for (int vertIndex = 0; vertIndex < 3; vertIndex++) {
      for (int i = 0; i < 3; i++) {
        vertices.push_back(
            tempGeom.vertices[(3 * face.vertexIndex[vertIndex]) + i]);
      }
      ...
      if (hasNormals) {
        for (int i = 0; i < 3; i++) {
          normals.push_back(
              tempGeom.normals[(3 * face.normalIndex[vertIndex]) + i]);
        }
      }
    }
    ...
    if (hasTextureCoords && hasNormals) {
      ...
      float bitangentLength =
          sqrt(bitangentX * bitangentX + bitangentY * bitangentY +
  ...
  cout << "Successfully loaded an OBJ with " << vertices.size() / 3
       << " vertices " << endl;
}

glm::vec3 GeometryData::center() {
  glm::vec3 objectCenter = glm::vec3(0.0f, 0.0f, 0.0f);
  for (size_t i = 0; i < vertexCount(); i++) {
    objectCenter +=
        glm::vec3(vertices[i * 3], vertices[i * 3 + 1], vertices[i * 3 + 2]);
  }
  objectCenter /= vertexCount();
  return objectCenter;
}

void GeometryData::boundingBox(glm::vec3 &minBounds, glm::vec3 &maxBounds) {
  if (vertexCount() == 0) {
  ...
  maxBounds = glm::vec3(vertices[0], vertices[1], vertices[2]);
...
int GeometryData::vertexCount() { return vertices.size() / 3; }

void *GeometryData::vertexData() { return (void *)&vertices[0]; }

void *GeometryData::textureCoordData() { return (void *)&textureCoords[0]; }

void *GeometryData::normalData() { return (void *)&normals[0]; }

void *GeometryData::tangentData() { return (void *)&tangents[0]; }

void *GeometryData::bitangentData() { return (void *)&bitangents[0]; }
